  ...
  const handleFilterSubmit = (e) => {
    ...
    if (filterCriteria.appointmentId) {
      filteredData = filteredData.filter((appointment) =>
        appointment.appointmentId.toString().includes(filterCriteria.appointmentId)
      );
    }

    if (filterCriteria.patientId) {
      filteredData = filteredData.filter((appointment) =>
        appointment.patient.patientId.toString().includes(filterCriteria.patientId)
      );
    }

    if (filterCriteria.startTo) {
      filteredData = filteredData.filter((appointment) =>
        appointment.startTo.includes(filterCriteria.startTo)
      );
    }

    if (filterCriteria.physicianId) {
      filteredData = filteredData.filter((appointment) =>
        appointment.physician.physicianId.toString().includes(filterCriteria.physicianId)
      );
    }

    if (filterCriteria.nurseId) {
      filteredData = filteredData.filter((appointment) =>
        appointment.nurse.nurseId.toString().includes(filterCriteria.nurseId)
      );
    }

    if (filterCriteria.examinationRoom) {
      filteredData = filteredData.filter((appointment) =>
        appointment.examinationRoom.toLowerCase().includes(filterCriteria.examinationRoom.toLowerCase())
      );
    }
...
>>>>>>> ed39860bc81b1a52067ef39b7f5b9283a95e0dcc
                </Link>
              </td>
